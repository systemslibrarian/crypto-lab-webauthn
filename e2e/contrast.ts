 ...
 *
 *  - TEXT OVER A SURFACE axe DECLINES TO RESOLVE. Every status pill on this
 *    page is a translucent `rgba()` tint — `--pass-bg` is
 *    `rgba(111,240,182,.13)`, `--fail-bg` is `rgba(255,110,134,.15)` — so the
 *    colour under a `pass`/`fail` badge depends entirely on what it is sitting
 *    on, and this lab stacks surfaces three deep: `--panel` on the section,
 *    `--panel-strong` on the output card inside it, `--bg-soft` on the
 *    `.compare-col` and `.check-row` inside that. The same badge is painted at
 *    all three depths (single result, compare column, live-demo panel). Several
 *    borders are `color-mix()`, which Chromium reports to `getComputedStyle`
 *    unresolved, and axe files anything it cannot resolve under `incomplete`
 ...
 *    `opacity: .85` and sets no colour of its own, so what it actually paints
 *    is only visible to a computed-style probe. Disabled buttons render at
 *    `.5`, which WCAG exempts and this helper honours below.
 *
 ...
 * rather than against white. A gradient is judged at the text's own position.
 *
 ...
 * not behind it. This page has exactly that: `.skip-link` and `.theme-toggle`
 * are `position: absolute`, and the parked skip link sits above the top of the
 * page entirely. So an ancestor's own paint is applied only when its border box
 ...
 *    boxes are the `.table-wrap` scrollers around every `.math-table` — the
 *    credential table, the verifier-context table, the ceremony table and the
 *    passwords-vs-passkeys table, each with `min-width: 320px` inside an
 *    `overflow-x: auto` wrapper, so at a 380px viewport the right-hand columns
 *    are genuinely outside their scroller's client box. Content scrolled past
 ...
 *    fall through to white, inventing a failure. Skip it, and rely on the cells
 ...
 *    mismeasures the first labelled diagram anyone adds: the ceremony diagram is
 *    the obvious candidate to redraw as SVG, and an ancestor-only walk would
...
export async function auditContrast(page: Page): Promise<ContrastFailure[]> {
  return page.evaluate(() => {
     ...
     * background is `color-mix(in oklab, var(--accent) 6%, transparent)` and
     * several check-row borders are `color-mix(in srgb, …)`, and Chromium
     * reports those to `getComputedStyle` unchanged rather than converting them
     * to sRGB. A regex that only understands `rgb()/rgba()` sees `null` for
     * every one of them and the walk then falls through to the wrong backdrop —
     * which fabricates failures (dark-on-light read as dark-on-dark) and, far
     ...
     * its worst stop, because a stop only covers the text if it happens to span
     * the element. On this page the one gradient that carries text is
     * `.hero-panel`'s `linear-gradient(135deg, var(--panel), var(--panel-strong))`,
     * whose two ends differ by enough to move a ratio. A non-gradient layer
     ...
     *
     * A driven scan here walks four `.math-table`s, ten `.lab-section`s of
     * prose, two `.compare-col` result blocks each holding its own six-row
     * `.check-list` and `.signed-bytes` definition list, and the live-demo
     * panel — and every one of those nodes re-walks the same handful of
     * ancestors up to `<body>`. Without the caches the pass re-reads the same
     * computed styles and rects thousands of times, and the gate scans twenty
     * states per configuration. Nothing mutates the DOM during the pass, so the
     ...
     * that is the `.table-wrap` scrollers around the four `.math-table`s and
     * the `.visually-hidden` captions clipped to a 1×1 box.
     */
    ...
    const svgUnderlay = (el: Element, box: DOMRect): RGBA => {
      ...
      // ratio was 6.15:1. The shared header's hamburger is drawn as exactly that
      // kind of stroke-only `<line>` geometry, so the guard is live on this page
      // even though no SVG text sits above it today.
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // Content hidden by `content-visibility` or a closed <details> keeps its
      // last laid-out geometry in Chromium, so the `display`/rect tests above
      // all pass for text that paints nothing. `checkVisibility()` catches it.
     ...
     * failure the spec does not actually require fixing. This page relies on it:
     * `button:disabled { opacity: .5 }`, and the live demo's "Authenticate with
     * the real passkey" button ships disabled until a real credential exists.
     */
     ...
     * it, the decoration this lab carries — the shared header's hamburger and
     * GitHub glyphs, the 🔐/🛡️ actor icons, the attack buttons' emoji — would be
     ...
     * BOTH oracles, so nothing here may hide a real value behind that attribute.
     * The gate asserts that separately — see `expectNoLiveTextAriaHidden`.
     */
     ...
     *
     * This lab has no SVG figure today — the ceremony diagram is built from
     * divs. The guard stays because the mistake is cheap to make and silent: a
     * `<svg>` whose markup is assembled by string interpolation picks up stray
     * whitespace and comma text nodes between its shapes, and a walk that
    ...
    for (const el of Array.from(document.querySelectorAll('body *'))) {
      ...
      const svgText = el.namespaceURI === SVG_NS;
